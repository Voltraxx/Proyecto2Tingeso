...
import main.core.Backend.Services.CreditRequestService;
import main.core.Backend.Services.CreditRequestService.LoanConditions;
...
public class CreditRequestController {
    ...
    @GetMapping("/{id}/evaluate/{rule}")
    public ResponseEntity<Boolean> evaluateRule(@PathVariable Long id, @PathVariable String rule) {
        CreditRequest creditRequest = creditRequestService.getCreditRequestById(id);
        boolean result = creditRequestService.evaluateRule(creditRequest, rule);
    ...
    @PostMapping("/{id}/calculateTotalCost")
        public ResponseEntity<Map<String, Object>> calculateTotalCost(
            ...
            Double seguroDesgravamen = values.getOrDefault("seguroDesgravamen", 0.0);
            Double seguroIncendio = values.getOrDefault("seguroIncendio", 0.0);
            Double comisionAdministracion = values.getOrDefault("comisionAdministracion", 0.0);

            Map<String, Object> result = creditRequestService.calculateTotalCost(creditRequest, seguroDesgravamen, seguroIncendio, comisionAdministracion);
            return ResponseEntity.ok(result);
        }
    ...
    public ResponseEntity<Void> updateCreditRequestStatus(@PathVariable Long id, @RequestBody Map<String, String> payload) {
        String action = payload.get("action");
        creditRequestService.updateStatusBasedOnAction(id, action);
